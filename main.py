...
def main():
    ...
    print('Total trainable params: {}'.format(pytorch_total_trainable_params))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint({
            ...
            'args': vars(args)
        }, is_best,  os.path.join(pwd, 'checkpoint.pth.tar'))

    tensorboard_writer.flush()
    tensorboard_writer.close()
    # test best model
    print('---------Evaluate Model on Test Set---------------')
    best_checkpoint = torch.load(os.path.join(pwd, 'model_best.pth.tar'))
    model.load_state_dict(best_checkpoint['state_dict'])
    validate(test_loader, model, criterion, epoch, normalizer, test=True)
    global_end_time = time.time()
    print('Total Running time is: {:.2f}s'.format(global_end_time - global_start_time))
...
def validate(val_loader, model, criterion, epoch, normalizer, test=False):
    ...
    if test:
        star_label = '**'
        import csv
        with open(os.path.join(pwd, 'test_results.csv'), 'w') as f:
            writer = csv.writer(f)
...
def save_checkpoint(state, is_best, filename):
    ...
    if is_best:
        shutil.copyfile(filename, os.path.join(pwd, 'model_best.pth.tar'))
